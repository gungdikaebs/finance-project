...
<template>
  <div
    class="min-h-[100dvh] bg-[#F3F5EF] dark:bg-[#0E1410] flex items-center justify-center p-4 relative overflow-hidden transition-colors duration-200">
    <!-- Ambient subtle background glow -->
    <div
      class="absolute -top-32 left-1/2 -translate-x-1/2 w-96 h-96 bg-[#183D2B]/8 dark:bg-[#B8DF38]/5 rounded-full blur-3xl pointer-events-none">
    </div>
    <div
      class="absolute -bottom-32 left-1/3 w-80 h-80 bg-[#B8DF38]/10 dark:bg-[#183D2B]/20 rounded-full blur-3xl pointer-events-none">
    </div>

    <div
      class="fintech-card bg-white dark:bg-[#16201A] w-full max-w-md rounded-2xl shadow-xl border border-emerald-950/10 dark:border-[#243329] p-7 sm:p-8 relative z-10 animate-in fade-in zoom-in-95 duration-200">
      <!-- Header / Brand -->
      <div class="text-center mb-7">
        <div
          class="inline-flex items-center justify-center w-13 h-13 rounded-2xl bg-gradient-to-br from-[#183D2B] via-[#143425] to-[#0D2218] dark:from-[#132E21] dark:to-[#0A1B13] text-[#B8DF38] font-bold text-xl mb-3.5 shadow-md ring-4 ring-[#B8DF38]/20 tracking-wider border border-[#B8DF38]/30">
          PK
        </div>
        <h1 class="text-2xl font-bold tracking-tight text-[#18221B] dark:text-[#F0F4F1]">Project-Keuangan</h1>
        <p class="text-xs text-[#18221B]/60 dark:text-[#98A79D] mt-1">Sistem Pengelolaan Keuangan Pribadi yang Terarah
        </p>
      </div>

      <!-- Mode Switcher Tabs -->
      <div
        class="flex rounded-xl bg-stone-200/50 dark:bg-[#0E1410] p-1 mb-6 border border-emerald-950/10 dark:border-[#243329]">
        <button type="button" @click="isRegister = false; errorMessage = ''; successMessage = '';" :class="[
        ...
        <button type="button" @click="isRegister = true; errorMessage = ''; successMessage = '';" :class="[
          'tactile-btn flex-1 py-2 text-xs font-semibold rounded-lg transition-all cursor-pointer',
          isRegister
            ? 'bg-white dark:bg-[#16201A] text-[#18221B] dark:text-[#F0F4F1] shadow-2xs'
            : 'text-[#18221B]/60 dark:text-[#98A79D] hover:text-[#18221B] dark:hover:text-[#F0F4F1]'
      ...
      <form @submit.prevent="handleSubmit" class="space-y-4">
        <div v-if="isRegister">
          <label
            class="block text-[11px] font-bold text-[#18221B]/70 dark:text-[#98A79D] uppercase tracking-wider mb-1.5">
            Nama Lengkap
          </label>
          <div class="relative">
            <User
              class="w-4 h-4 absolute left-3.5 top-1/2 -translate-y-1/2 text-[#18221B]/40 dark:text-[#98A79D]/60 pointer-events-none" />
            <input v-model="name" type="text" placeholder="Misal: Enami Asa"
        ...
        <div>
          ...
          <div v-if="isRegister && password"
            class="mt-2.5 p-3 rounded-xl bg-stone-50 dark:bg-[#0E1410] border border-stone-200 dark:border-[#243329] space-y-2">
            ...
            <div class="w-full h-1.5 bg-stone-200 dark:bg-stone-800 rounded-full overflow-hidden">
              <div class="h-full transition-all duration-300 rounded-full" :class="passwordStrengthMeta.color"
                :style="{ width: passwordStrengthMeta.width }"></div>
            </div>
