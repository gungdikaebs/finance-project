...
<template>
  ...
  >
    ...
    <!-- Modal Container -->
    <div
      class="relative w-full max-w-lg bg-white rounded-t-3xl sm:rounded-3xl max-h-[92dvh] sm:max-h-[85vh] shadow-2xl border border-emerald-950/10 overflow-hidden flex flex-col mx-auto animate-modal-enter"
    >
      <!-- Top Decorative Accent -->
      <div class="h-2 w-full bg-gradient-to-r from-[#183D2B] via-[#B8DF38] to-[#183D2B]"></div>

      <!-- Header: Step Indicators & Title (shrink-0) -->
      <div class="px-5 sm:px-6 pt-5 pb-4 border-b border-stone-100 shrink-0">
        <div class="flex items-center justify-between mb-3.5">
          <div class="flex items-center gap-2.5">
            <div class="w-8 h-8 rounded-xl bg-[#183D2B] text-[#B8DF38] flex items-center justify-center font-bold text-xs shadow-sm">
              <Sparkles class="w-4 h-4" />
            </div>
            <div>
              <span class="text-xs font-semibold uppercase tracking-wider text-[#183D2B]/70">
                Panduan Pengguna Baru
              </span>
              <h2 class="text-lg font-bold text-[#18221B] leading-tight">
      ...
      <div class="p-5 sm:p-6 overflow-y-auto flex-1 overscroll-contain">
        ...
        <div
          v-if="errorMessage"
          class="mb-4 p-3 bg-rose-50 border border-rose-200 text-rose-800 rounded-xl text-xs flex items-center gap-2"
        >
          <Info class="w-4 h-4 shrink-0 text-rose-600" />
        ...
        <div v-if="currentStep === 1" class="space-y-4 animate-in fade-in slide-in-from-right-4 duration-200">
          <div class="w-14 h-14 rounded-2xl bg-emerald-50 border border-emerald-200/60 text-[#183D2B] flex items-center justify-center mx-auto shadow-sm">
            <Layers class="w-7 h-7 text-[#183D2B]" />
          </div>
          <div class="text-center space-y-1.5">
            <h3 class="text-base font-bold text-[#18221B]">
              Filosofi "Satu Saldo Utama"
            </h3>
            <p class="text-xs text-stone-600 leading-relaxed max-w-sm mx-auto">
          ...
          <div class="bg-[#F3F5EF] rounded-2xl p-4 border border-emerald-950/5 space-y-2.5 text-xs text-stone-700">
            <div class="flex items-start gap-2.5">
              <CheckCircle2 class="w-4 h-4 text-[#183D2B] shrink-0 mt-0.5" />
        ...
        <div v-if="currentStep === 2" class="space-y-4 animate-in fade-in slide-in-from-right-4 duration-200">
          <div class="w-14 h-14 rounded-2xl bg-emerald-50 border border-emerald-200/60 text-[#183D2B] flex items-center justify-center mx-auto shadow-sm">
            <Wallet class="w-7 h-7 text-[#183D2B]" />
          </div>
          <div class="text-center space-y-1.5">
            <h3 class="text-base font-bold text-[#18221B]">
          ...
          <div class="space-y-2">
            ...
            <div class="relative">
              ...
              <input
                ...
                placeholder="0"
                class="w-full pl-12 pr-4 py-3.5 bg-[#F3F5EF] border border-stone-200 rounded-xl text-lg font-bold text-[#18221B] tabular-nums focus:outline-none focus:ring-2 focus:ring-[#B8DF38] focus:border-[#183D2B] transition-all"
                autofocus
              />
            </div>
            <p class="text-[11px] text-stone-500 flex items-center gap-1.5">
              <Info class="w-3.5 h-3.5 shrink-0 text-stone-400" />
        ...
        <div v-if="currentStep === 3" class="space-y-4 animate-in fade-in slide-in-from-right-4 duration-200">
          <div class="w-14 h-14 rounded-2xl bg-emerald-50 border border-emerald-200/60 text-[#183D2B] flex items-center justify-center mx-auto shadow-sm">
            <ReceiptText class="w-7 h-7 text-[#183D2B]" />
          </div>
          <div class="text-center space-y-1.5">
            <h3 class="text-base font-bold text-[#18221B]">
        ...
        <div v-if="currentStep === 4" class="space-y-4 animate-in fade-in slide-in-from-right-4 duration-200">
          <div class="w-14 h-14 rounded-2xl bg-emerald-50 border border-emerald-200/60 text-[#183D2B] flex items-center justify-center mx-auto shadow-sm">
            <ShieldAlert class="w-7 h-7 text-[#183D2B]" />
          </div>
          <div class="text-center space-y-1.5">
            <h3 class="text-base font-bold text-[#18221B]">
          ...
          <div class="grid grid-cols-1 gap-2.5">
            <button
              ...
              class="p-3.5 rounded-2xl border text-left flex items-center justify-between transition-all tactile-btn"
              :class="[
                emergencyMonths === opt.value
                  ? 'border-[#183D2B] bg-emerald-50/70 ring-2 ring-[#B8DF38]/50 shadow-sm'
                  : 'border-stone-200 bg-white hover:border-stone-300'
              ]"
            >
              <div>
                <div class="text-xs font-bold text-[#18221B]">{{ opt.label }}</div>
                <div class="text-[11px] text-stone-500">{{ opt.desc }}</div>
              </div>
              <div
                class="w-5 h-5 rounded-full flex items-center justify-center border transition-all"
                :class="[
                  emergencyMonths === opt.value
                    ? 'border-[#183D2B] bg-[#183D2B] text-[#B8DF38]'
          ...
          <!-- Target Preview Box -->
          <div class="p-3.5 bg-[#F3F5EF] rounded-2xl border border-emerald-950/10 flex items-center justify-between">
            <span class="text-xs text-stone-600 font-medium">Target Nominal Dana Pengaman:</span>
            <span class="text-sm font-bold text-[#183D2B] tabular-nums">
        ...
        <div v-if="currentStep === 5" class="space-y-4 animate-in fade-in slide-in-from-right-4 duration-200">
          <div class="w-14 h-14 rounded-2xl bg-emerald-50 border border-emerald-200/60 text-[#183D2B] flex items-center justify-center mx-auto shadow-sm">
            <PieChart class="w-7 h-7 text-[#183D2B]" />
          </div>
          <div class="text-center space-y-1.5">
            <h3 class="text-base font-bold text-[#18221B]">
          ...
          <div class="space-y-2 text-xs">
            <div class="p-3 bg-stone-50 rounded-xl border border-stone-200/80 flex items-center justify-between">
              <span class="text-stone-600">Saldo Awal Riil:</span>
              <span class="font-bold text-[#18221B] tabular-nums">{{ formatRupiah(rawInitialBalance) }}</span>
            </div>
            <div class="p-3 bg-stone-50 rounded-xl border border-stone-200/80 flex items-center justify-between">
              <span class="text-stone-600">Kebutuhan Pokok Bulanan:</span>
              <span class="font-bold text-[#18221B] tabular-nums">{{ formatRupiah(rawMonthlyNeeds) }}</span>
            </div>
            <div class="p-3 bg-stone-50 rounded-xl border border-stone-200/80 flex items-center justify-between">
              <span class="text-stone-600">Target Dana Pengaman ({{ emergencyMonths }} Bln):</span>
              <span class="font-bold text-[#183D2B] tabular-nums">{{ formatRupiah(targetEmergencyAmount) }}</span>
            </div>
          </div>
          ...
          <div class="space-y-1.5 pt-1">
            <div class="flex items-center justify-between text-[11px] font-bold">
              <span class="text-emerald-800">50% Kebutuhan</span>
              <span class="text-lime-700">30% Tabungan</span>
              <span class="text-amber-700">20% Keinginan</span>
            </div>
            <div class="h-2.5 w-full rounded-full overflow-hidden flex bg-stone-200 shadow-inner">
              <div class="bg-[#183D2B] h-full w-[50%]"></div>
      ...
      <!-- Footer Actions -->
      <div class="px-5 sm:px-6 py-4 bg-stone-50/80 border-t border-stone-100 flex items-center justify-between shrink-0 gap-3">
        ...
        <div>
          <button
            ...
            type="button"
            @click="prevStep"
            class="min-h-[44px] px-4 py-2.5 rounded-xl border border-stone-200 text-xs font-semibold text-stone-600 hover:bg-stone-100 transition-all flex items-center gap-1.5 tactile-btn cursor-pointer"
          >
          ...
          <button
            ...
            :disabled="saving"
            class="min-h-[44px] flex items-center text-xs font-semibold text-stone-500 hover:text-stone-800 transition-colors py-2 px-1 cursor-pointer"
          >
        ...
        <div class="flex items-center gap-2">
          <button
            ...
            type="button"
            @click="nextStep"
            class="min-h-[44px] px-5 py-2.5 rounded-xl bg-[#183D2B] text-white hover:bg-emerald-900 text-xs font-bold transition-all shadow-sm flex items-center gap-1.5 tactile-btn cursor-pointer"
          >
            Lanjut
