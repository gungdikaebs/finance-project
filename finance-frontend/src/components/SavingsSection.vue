...
const expandedGoalIds = ref(new Set<number>());

const isGoalExpanded = (goalId: number): boolean => expandedGoalIds.value.has(goalId);

const toggleGoalDetails = (goalId: number) => {
  const expanded = new Set(expandedGoalIds.value);
...
const emergencyTargetMet = computed(() =>
  emergencyTargetNominal.value > 0n &&
  BigInt(props.emergencyGoal?.currentBalance || '0') >= emergencyTargetNominal.value,
);

const emergencyExcess = computed(() => {
  if (!emergencyTargetMet.value) return '0';
  return (BigInt(props.emergencyGoal?.currentBalance || '0') - emergencyTargetNominal.value).toString();
});
...
const getGoalExcess = (goal: SavingsGoal): string => {
  const balance = BigInt(goal.currentBalance || '0');
  const target = BigInt(getGoalTargetPrice(goal) || '0');
  return balance > target ? (balance - target).toString() : '0';
};
    ...
    <div class="flex flex-col gap-3 border-b border-stone-100 dark:border-[#243329] pb-4">
      <div>
        ...
        <p class="text-xs text-[#5E6961] dark:text-[#98A79D] mt-1 font-normal">
          ...
          <template v-else>
            Setoran dibagi: 60% untuk Dana Pengaman dan 40% untuk Target Impian.
          </template>
        </p>
      </div>
          ...
          >
            <div class="flex flex-wrap items-start justify-between gap-2">
              ...
              >
                ...
                <span>Harga naik lebih cepat</span>
              </span>
              <span
